...
var (
	...
	nonceAuthVote = hexutil.MustDecode("Gstffffffffffffffff") // Magic nonce number to vote on adding a new signer
	nonceDropVote = hexutil.MustDecode("Gst0000000000000000") // Magic nonce number to vote on removing a signer.
...
func (c *Clique) Seal(chain consensus.ChainReader, block *types.Block, results chan<- types.SealResult, stop <-chan struct{}) error {
	header := block.Header()
